...
hist(iris$Sepal.Length, main= '붓꽃 꽃받침 길이', col='orange') # 히스토그램램
...
## 정책 지지도 그래프
par(mfrow=c(2,3), mar=c(3,3,4,2))  # 화면을 2행 3열의 레이아웃으로 분할
ds$region
table(ds$region) # 2700행을(C M N S SA) 취합, 하나의 테이블로 생성

ds_region <- table(ds$region)
ds_gender <- table(ds$sex)
ds_edu <- table(ds$education)

barplot(table(ds$region), main = '지역별 분포', col=colors[1:5])   # 방법1- 테이블 사용 table(ds$region)
barplot(ds_gender, main = '성별 분포', col=colors[5:6])      # 방법2 - 따로 생성 ds_gender <- table(ds$sex)
barplot(ds_edu, main = '교육수주별 분포', col=colors[7:9])

hist(ds$age, breaks = 6, main = '연령별', xlab = 'ages', col=colors[10:15])
hist(ds$income, breaks = 4, main='수입별', col=colors[15:18])
hist(ds$statusquo, breaks = 9, main = '정책지지도', col=colors[11:19])

par(mfrow=c(1,1))  # 설정했던 화면의 레이아웃을 초기 상태로 되돌림
...
hist(tips.new$perHead, breaks = 5, main='1인당 주문액', xlab = '단위:$', ylab = '빈도수', las=2, col=color.5)

hist(Lunch.perHead, breaks = 4, main ='1인당 점심 주문액', xlab = '단위:$', ylab = '빈도수', las=2, col=color.5)

hist(dinner.perHead, breaks = 4, main = '1인당 저녁 주문액', xlab = '단위:$', ylab = '빈도수', las=2, col=color.5)
...
res.lunch <- hist(tips.new[tips.new$time == 'Lunch', 'percent'],
                  main = '주문 금액 대비 팁의 비율(점심)', xlab = 'percent(%)', breaks = 5)
res.dinner <- hist(tips.new[tips.new$time == 'Dinner', 'percent'],
                  main = '주문 금액 대비 팁의 비율(저녁)', xlab = 'percent(%)', breaks = 5)
...
barplot(res, main='식사시간에 따른 팁비율 분포', col=color.2, legend.txt = c('Lunch', 'Dinner'), las =2)
